...
export default function Dashboard() {
  ...
  const {
    ...
    queryFn: async () => {
      const {
        data,
        error
      } = await supabase.from('course_progress').select(`
          *,
          courses (
            id,
            title,
  ...
  const {
    ...
    queryFn: async () => {
      const {
        data,
        error
      } = await supabase.from('learning_goals').select('*').eq('user_id', user!.id).eq('status', 'active').limit(3);
      if (error) throw error;
      return data;
    },
    enabled: !!user
  ...
  const {
    ...
    queryFn: async () => {
      const {
        data,
        error
      } = await supabase.from('learning_sessions').select(`
          *,
          courses (title)
  ...
  const {
    ...
    queryFn: async () => {
      const {
        data,
        error
      } = await supabase.from('learning_goals').select('*').eq('user_id', user!.id).order('created_at', {
        ascending: false
      });
      if (error) throw error;
      return data;
  ...
  const {
    ...
    queryFn: async () => {
      const {
        data,
        error
      } = await supabase.from('user_rewards').select('*').eq('user_id', user!.id).order('earned_at', {
        ascending: false
      });
      if (error) throw error;
      return data;
  ...
  const {
    ...
    queryFn: async () => {
      const {
        data,
        error
      } = await supabase.from('learning_materials').select(`
          *,
          courses (title)
  ...
  const chartData = recentSessions?.map((session: any) => ({
    date: new Date(session.session_date).toLocaleDateString('id-ID', {
      day: 'numeric',
      month: 'short'
    }),
  ...
  const totalProgress = courseProgress?.reduce((acc, cp: any) => acc + calculateProgress(cp.completed_modules, cp.courses.total_modules), 0) || 0;
  const averageProgress = courseProgress?.length ? Math.round(totalProgress / courseProgress.length) : 0;
  ...
  const uploadMaterialMutation = useMutation({
    mutationFn: async () => {
      ...
      const ALLOWED_TYPES = ['application/pdf', 'application/msword', 'application/vnd.openxmlformats-officedocument.wordprocessingml.document', 'application/vnd.ms-powerpoint', 'application/vnd.openxmlformats-officedocument.presentationml.presentation', 'application/vnd.ms-excel', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', 'text/plain'];
      ...
      const {
        error: uploadError
      } = await supabase.storage.from('learning-materials').upload(filePath, selectedFile);
  ...
  const getSessionsForDate = (date: Date) => {
    return recentSessions?.filter((session: any) => isSameDay(new Date(session.session_date || session.created_at), date));
  };
  ...
  const filteredMaterials = materials?.filter((material: any) => material.title.toLowerCase().includes(searchQuery.toLowerCase()) || material.description?.toLowerCase().includes(searchQuery.toLowerCase()));
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        <div className="grid gap-6 md:grid-cols-2 mb-8">
          <Card className="shadow-md">
            ...
            <CardContent>
              {chartData.length > 0 ? <ResponsiveContainer width="100%" height={200}>
                ...
                </ResponsiveContainer> : <p className="text-center text-muted-foreground py-8">Belum ada data</p>}
            </CardContent>
          </Card>
          ...
          <Card className="shadow-md">
            ...
            <CardContent>
              {goals && goals.length > 0 ? <div className="space-y-3">
                  {goals.slice(0, 3).map((goal: any) => <div key={goal.id} className="p-3 rounded-lg bg-gradient-card">
                ...
                </div> : <p className="text-center text-muted-foreground py-8">Belum ada target aktif</p>}
            </CardContent>
          </Card>
        </div>
        ...
        <Card className="shadow-md mb-8">
                ...
                <DialogContent>
                  ...
                  <form onSubmit={handleGoalSubmit} className="space-y-4">
                    <div>
                      ...
                      <Input id="title" value={goalFormData.title} onChange={e => setGoalFormData({
                      ...goalFormData,
                    ...
                    <div>
                      ...
                      <Textarea id="description" value={goalFormData.description} onChange={e => setGoalFormData({
                      ...goalFormData,
                    ...
                    <div>
                      ...
                      <Select value={goalFormData.goal_type} onValueChange={value => setGoalFormData({
                      ...goalFormData,
                      goal_type: value
                    })}>
                        <SelectTrigger>
                    ...
                    <div>
                      ...
                      <Input id="target_value" type="number" min="1" value={goalFormData.target_value} onChange={e => setGoalFormData({
                      ...goalFormData,
                    ...
                    <div>
                      ...
                      <Input id="end_date" type="date" value={goalFormData.end_date} onChange={e => setGoalFormData({
                      ...goalFormData,
          ...
          <CardContent>
            {allGoals && allGoals.length > 0 ? <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                {allGoals.map((goal: any) => {
              const progress = goal.current_value / goal.target_value * 100;
              return <Card key={goal.id} className="shadow-sm">
                      <CardHeader>
                      ...
                      <CardContent className="space-y-4">
                        ...
                        <div className="space-y-2">
                          ...
                          <div className="flex gap-2">
                            <Button size="sm" variant="outline" onClick={() => updateProgressMutation.mutate({
                        id: goal.id,
                        current_value: Math.max(0, goal.current_value - 1)
                      })}>
                              -
                            </Button>
                            <Button size="sm" className="flex-1" onClick={() => updateProgressMutation.mutate({
                        id: goal.id,
                        current_value: Math.min(goal.target_value, goal.current_value + 1)
                      })}>
                              + 1
                            </Button>
                          </div>
        ...
        <Card className="shadow-md mb-8">
          <CardHeader>
            <CardTitle>Waktu Pembelajaran</CardTitle>
            ...
            <div className="grid gap-6 md:grid-cols-2">
              <div className="space-y-6">
                ...
                <div className="space-y-3">
                  <p className="text-sm font-medium">Pilihan Waktu:</p>
                  <div className="grid grid-cols-2 gap-2">
                    <Button variant="outline" onClick={() => setPresetTime(25, 'study')} disabled={isActive}>
                      Belajar 25m
                    </Button>
                    <Button variant="outline" onClick={() => setPresetTime(5, 'break')} disabled={isActive}>
                      Istirahat 5m
                    </Button>
                    <Button variant="outline" onClick={() => setPresetTime(50, 'study')} disabled={isActive}>
                      Belajar 50m
                    </Button>
                    <Button variant="outline" onClick={() => setPresetTime(10, 'break')} disabled={isActive}>
                      Istirahat 10m
                    </Button>
                  </div>
                </div>
            ...
            <div className="grid gap-6 md:grid-cols-2">
              <div>
                ...
                <div className="grid grid-cols-7 gap-2">
                  {['Min', 'Sen', 'Sel', 'Rab', 'Kam', 'Jum', 'Sab'].map(day => <div key={day} className="text-center text-xs font-medium text-muted-foreground p-2">
                      {day}
                    </div>)}
                  {daysInMonth.map(date => {
                  const sessionsForDate = getSessionsForDate(date);
                  const hasActivity = sessionsForDate && sessionsForDate.length > 0;
                  return <button key={date.toString()} onClick={() => setSelectedDate(date)} className={`
              ...
              <div>
                ...
                {recentSessions && recentSessions.length > 0 ? <div className="space-y-3 max-h-[400px] overflow-y-auto">
                    {recentSessions.slice(0, 10).map((session: any) => <div key={session.id} className="p-3 rounded-lg bg-gradient-card border border-border">
        ...
        <Card className="shadow-md mb-8">
          <CardHeader>
            <div className="flex items-center justify-between">
              ...
              {isAdmin && <Dialog open={isMaterialDialogOpen} onOpenChange={setIsMaterialDialogOpen}>
                  <DialogTrigger asChild>
                    <Button className="gap-2">
                      <Plus className="h-4 w-4" />
                      Unggah Materi
                  ...
                  <DialogContent>
                    ...
                    <form onSubmit={handleMaterialSubmit} className="space-y-4">
                      <div>
                        ...
                        <Input id="material_title" value={materialFormData.title} onChange={e => setMaterialFormData({
                      ...materialFormData,
                      title: e.target.value
                    })} required />
                      </div>
                      <div>
                        ...
                        <Textarea id="material_description" value={materialFormData.description} onChange={e => setMaterialFormData({
                      ...materialFormData,
                      description: e.target.value
                    })} />
                      </div>
                      <div>
                        ...
                        <Select value={materialFormData.course_id} onValueChange={value => setMaterialFormData({
                      ...materialFormData,
                      course_id: value
                    })}>
                          <SelectTrigger>
          ...
          <CardContent>
            <div className="mb-4">
              <div className="relative">
                ...
                <Input placeholder="Cari materi..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="pl-10" />
              </div>
            </div>
            {isLoadingMaterials ? <p className="text-center text-muted-foreground py-8">Memuat data...</p> : filteredMaterials && filteredMaterials.length > 0 ? <div className="grid gap-4">
                {filteredMaterials.slice(0, 5).map((material: any) => <Card key={material.id} className="shadow-sm">
                    <CardHeader>
                      <div className="flex items-start justify-between">
                        ...
                        <div className="flex gap-2">
                          {material.file_path && <Button variant="outline" size="sm" asChild>
                              <a href={material.file_path} download target="_blank" rel="noopener noreferrer">
                                <Download className="h-4 w-4" />
                              </a>
                            </Button>}
                          {isAdmin && <Button variant="ghost" size="sm" onClick={() => deleteMaterialMutation.mutate(material.id)}>
                              <Trash2 className="h-4 w-4 text-destructive" />
                            </Button>}
                        </div>
                      </div>
                    ...
                    <CardContent>
                      {material.description && <p className="text-sm text-muted-foreground mb-2">{material.description}</p>}
                      <div className="flex gap-4 text-xs text-muted-foreground">
                        {material.file_size && <span>Ukuran: {formatFileSize(material.file_size)}</span>}
                        <span>
                ...
                <p className="text-muted-foreground">
                  {searchQuery ? 'Tidak ada materi yang cocok dengan pencarian' : 'Belum ada materi tersedia'}
                </p>
              </div>}
          </CardContent>
        </Card>
        ...
        <Card className="shadow-md mb-8">
          ...
          <CardContent>
            ...
            {isLoadingRewards ? <p className="text-center text-muted-foreground py-8">Memuat data...</p> : rewards && rewards.length > 0 ? <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                {rewards.slice(0, 6).map((reward: any) => <Card key={reward.id} className="shadow-sm">
                    <CardHeader>
                      <div className={`w-12 h-12 rounded-full ${getBadgeColor(reward.badge_type)} flex items-center justify-center text-white mb-2`}>
        ...
        <Card className="shadow-md">
          ...
          <CardContent>
            {isLoading ? <p className="text-center text-muted-foreground py-8">Memuat data...</p> : courseProgress && courseProgress.length > 0 ? <div className="space-y-6">
              ...
              return <div key={cp.id} className="space-y-3 p-4 rounded-lg bg-gradient-card">
                      <div className="flex items-start justify-between">
